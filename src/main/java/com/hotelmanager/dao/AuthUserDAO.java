...
/**
 * Authentication DAO - Uses SQLite for login/registration operations only
 * This provides local authentication capability independent of the main business database
 */
public class AuthUserDAO {
    
    private Connection getConnection() throws SQLException {
        return AuthDBConnection.getInstance().getConnection();
    }
    ...
    public int createUser(User user) throws SQLException {
        String sql = "INSERT INTO users (username, password_hash, email, first_name, last_name, phone, role_id, is_active, security_question, security_answer_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
    ...
    public boolean updateUser(User user) throws SQLException {
        String sql = "UPDATE users SET username = ?, email = ?, first_name = ?, last_name = ?, phone = ?, role_id = ?, is_active = ?, security_question = ?, security_answer_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, user.getUsername());
    ...
    public boolean updateSecurityQuestion(int userId, String question, String answer) throws SQLException {
        String sql = "UPDATE users SET security_question = ?, security_answer_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public String generateRecoveryToken(int userId) throws SQLException {
        ...
        String sql = "UPDATE users SET recovery_token = ?, recovery_token_expires = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public User verifyRecoveryToken(String token) throws SQLException {
        String sql = "SELECT * FROM users WHERE recovery_token = ? AND recovery_token_expires > datetime('now')";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, token);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public boolean resetPasswordWithToken(String token, String newPassword) throws SQLException {
        ...
        String sql = "UPDATE users SET password_hash = ?, recovery_token = NULL, recovery_token_expires = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, PasswordUtil.hashPassword(newPassword));
    ...
    public boolean clearRecoveryToken(int userId) throws SQLException {
        String sql = "UPDATE users SET recovery_token = NULL, recovery_token_expires = NULL, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, userId);
            return ps.executeUpdate() > 0;
